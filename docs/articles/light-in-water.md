...
$n(\lambda) = E(\lambda)\,\lambda/(hc)$. The same constant underlies the
...
it uses
$E(z,\lambda) = E\left( z_{0},\lambda \right)\exp\left\lbrack - K_{d}(\lambda)\left( z - z_{0} \right) \right\rbrack$.
The calculation assumes that $K_{d}(\lambda)$ is constant over the whole
